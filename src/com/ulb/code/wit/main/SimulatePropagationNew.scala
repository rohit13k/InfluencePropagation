...
object SimulatePropagationNew {
  ...
  // val filelist = Array("slashdot-threads", "higgs-activity_time")
  ...
  def main(args: Array[String]): Unit =
    {
      ...
      for (file <- filelist) {
        val iFile = getData(folder + "input\\" + file + "_test_50.txt")
        ...
        for (k <- 0 to window.length - 1) {
          ...
          for (j <- 0 to seeds.length - 1) {
            ...
            var countFile = getData(folder + "groundtruth\\final\\" + file + "_outdegree_50.txt", seeds(j))
            result = simulatewithWindow(countFile, iFile, simulation, window(k).toLong, realwindow)
            print(", mean , " + Math.round(result._1.toDouble) + " ,sd ," + Math.round(result._2))

            var smartcountFile = getData(folder + "groundtruth\\final\\" + file + "_smartOutDegree_50.csv", seeds(j))
            result = simulatewithWindow(smartcountFile, iFile, simulation, window(k).toLong, realwindow)
            print(", mean , " + Math.round(result._1.toDouble) + " ,sd ," + Math.round(result._2))

            var skimFile = getData(folder + "groundtruth\\final\\" + file + "_50.keys", seeds(j))
            ...
            //            var exactkeyFile = getData(folder + "groundtruth\\new\\" + file + "_training_" + window(k) + "_50_exact.keys", seeds(j))
            //            //       var keyFile = folder + "exactoutput/" + file(i) + "_" + window(k) + "_" + seeds(j) + ".keys"
